...
From there:
- Home greets you by name and gives a quick-glance summary: how many ingredients are in your kitchen, how many are expiring soon, how many are on your shopping list, plus one-tap shortcuts into the Kitchen Timer, What Can I Make?, Shopping List and recipe search
...
- Profile shows your cooking stats at a glance - meals cooked, cuisines tried, a difficulty breakdown, and how many ingredients you've used up before they expired - with quick links into Saved Recipes and Cooking History
- Settings holds display name, dietary preferences and avoided ingredients, saved server-side so it survives a reinstall

Every screen from the original Figma is built and wired to a real ViewModel and, where it needs to be, the API. The one deliberate gap: recipe and profile imagery is icon-based rather than photos, since no blob storage is wired up for Part 2 - and a handful of features (biometric unlock, push notifications, offline mode, Afrikaans) are scoped for the POE, not Part 2 - see "Still to come" below.
...
- **Home** - a personalised landing screen: kitchen stats (in kitchen / expiring soon / to buy), an "expiring soon" strip pulled live from the pantry, a short list of recommended recipes, and one-tap shortcuts into the Kitchen Timer, What Can I Make?, Shopping List and recipe search
...
- **Profile** - cooking stats (meals cooked, cuisines, difficulty spread, ingredients saved from waste), with links into Saved Recipes and Cooking History, and a settings shortcut
- **Settings** - name, dietary preferences, avoided ingredients, all saved through the API

Every screen on that list is real and working, tied to a ViewModel and, where it needs to be, the API.
...
- **FR-02 (biometric unlock)** - not built yet, scoped for the POE.
...
- **FR-06 (expiry flagging)** - done. Anything expiring within three days gets pulled into its own group at the top of My Kitchen, and surfaced again on Home. No push notification for it yet - that part is FR-10.
...
- **FR-10 (notifications)** - not built yet, scoped for the POE.
...
- **FR-12 (offline via RoomDB)** - not built yet, scoped for the POE.
...
- Afrikaans
- Recipe and profile photos, once blob storage is wired up
